...
4. In Advanced settings, keep Python `3.11` (or use `runtime.txt` in this repo).
5. Deploy.

### Recommended Streamlit secrets/environment variables

The app now defaults to `USE_SIMPLE_LLM=true`, which works on Streamlit Cloud without a local Ollama server.
...
If you have a reachable Ollama endpoint, set:
